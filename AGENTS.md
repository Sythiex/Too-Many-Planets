...
Installed Factorio mod sources are available in `%APPDATA%\Factorio\mods` (equivalent to `C:\Users\<username>\AppData\Roaming\Factorio\mods`). Use the mod folders and archives there to verify internal mod IDs, prototype names, settings, and compatibility behavior.
